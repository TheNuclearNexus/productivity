    ...
    def refresh_apps(self):
        ...
        def get_app_score(app_data):
            ...
            try:
                ...
                matches = []
                for (cached_title, cached_profile), entry in self.engine.classifier._cache.items():
    ...
    def paintEvent(self, event):
        ...
        if self.apps and 0 <= self.selected_index < len(self.apps):
            ...
            if metrics.horizontalAdvance(display_name) > MAX_WIDTH:
                display_name = metrics.elidedText(name, Qt.TextElideMode.ElideRight, MAX_WIDTH)
                
            text_width = metrics.horizontalAdvance(display_name)
            painter.drawText(int(center.x() - text_width / 2), int(center.y() + 60), display_name)
        ...
        for ring_idx, app_data in enumerate(ring_apps):
            ...
            count_in_this_ring = count_inner if is_inner else len(ring_apps) - count_inner
            ...
            if pixmap:
                ...
                if ring_idx == self.selected_index:
                    painter.setPen(QPen(QColor(255, 255, 255, 220), 4, Qt.PenStyle.SolidLine))
                    painter.setBrush(Qt.BrushStyle.NoBrush)
