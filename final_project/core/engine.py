...
from final_project.monitors.window import WindowMonitor
from final_project.monitors.input import InputMonitor
from final_project.core.state import FocusState
from final_project.core.profile import FocusProfile
from final_project.llm.classifier import WindowClassifier
...
class TrackerEngine:
    ...
    def _bootstrap_classifications(self):
        ...
        print(f"Bootstrapping {len(apps)} background applications against Profile Context: {self.state.profile.name}")
    ...
    def tick(self):
        ...
        def background_classify():
            ...
            try:
                relevance, pretty_title = asyncio.run(self.classifier.classify(target_title, self.state.profile))
                
                window_switched = (target_title != self.last_window_title) and self.last_window_title != ""
