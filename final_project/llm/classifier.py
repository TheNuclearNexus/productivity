...
class WindowClassifier:
                ...
                for profile_name, apps in data.items():
                    for title, val in apps.items():
                        if isinstance(val, dict):
                            ...
                            self._cache[(title, profile_name)] = {"score": float(val), "pretty_name": title}
            except Exception:
                pass
    ...
    async def classify(self, window_title: str, profile: FocusProfile) -> Tuple[float, str]:
        """
        ...
        if self.overrides:
            ...
            if override_entry is not None:
                ...
                return override_entry.get("score", 0.5), override_entry.get("pretty_name", window_title)
        ...
        response = await self.client.generate(prompt=prompt, model=self.model, json_mode=True)
